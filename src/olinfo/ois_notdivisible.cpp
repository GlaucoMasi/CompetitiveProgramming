...
int main() {
  ...
  for(int i = 0; i < m; i++){
    ...
    if(a == b) quer[i] = {idx[a].first, idx[a].first, a, a, i};
    else if(idx[a].first < idx[b].first && idx[a].second > idx[b].second) quer[i] = {idx[a].first, idx[b].first, a, b, i};
    else{
  ...
  vector<bool> prime(maxv, 1);
  for(long long i = 2; i < maxv; i++){
    if(prime[i]){
      left.push(i);
      for(long long j = i*i; j < maxv; j += i) prime[j] = 0;
    }
  }
  ...
  auto add = [&](int pos){
    if(v[pos] >= maxv || !prime[v[pos]]) return;

    freq[v[pos]]++;
    while(freq[left.top()]) left.pop();
  };
  ...
  for(auto q : quer){
    ...
    int lca = calc(a, b);
    if(lca != a && lca != b) add(lca);
    ans[i] = left.top();
    if(lca != a && lca != b) remove(lca);
  }
