...
void print(node *treap){
  fout << treap->val << " " << treap->size << " " << treap->priority << endl;

  if(treap->l) fout << "left " << treap->l->val << endl;
  if(treap->r) fout << "right " << treap->r->val << endl;
  fout << endl;

  if(treap->l) print(treap->l);
  if(treap->r) print(treap->r);
}
